...
document.addEventListener("click", (event) => {
  const targetClass = event.target.classList.contains(`super_element`)
    ? "присутствует"
    : "отсутствует";
  console.log(
    `Класс "super_element" ${targetClass} в элементе "${event.target.tagName.toLowerCase()}".`
  );
});
...
// ОТВЕТ
// Так происходит по причине устройства Фазы Всплытия - распространяется через предков Цели в обратном порядке, начиная с Родителя Цели и заканчивая Окном.
